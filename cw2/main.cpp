...
#		define ASSETDIR_ "assets/cw2/"
		constexpr char const* kPBRPath = ASSETDIR_ "sponza-pbr.comp5822mesh";
#		undef ASSETDIR_
	...
	void record_commands(VkCommandBuffer, VkRenderPass, VkFramebuffer, VkPipeline, VkExtent2D const&, BakedModel& ret, VkBuffer aSceneUBO,
		glsl::SceneUniform const&, VkPipelineLayout, VkDescriptorSet aSceneDescriptors, std::vector<VkDescriptorSet> modelDescriptors, 
		BakedMeshBuffer& meshbuffer, VkPipeline);
	...
	glm::vec4 update_light_state(glm::vec3 lightpos, float aElapsedTime);
...
int main() try
{
	...
	lut::Pipeline pipe = create_baked_model_pipeline(window, renderpass.handle, pipeLayout.handle);
	...
	sceneUniforms.pointLight[0].lightPosition = glm::vec4(0.f, 2.f, 0.f, 1.f);
	...
	while (!glfwWindowShouldClose(window.window))
	{
		...
		if (recreateSwapchain)
		{
			...
			if (changes.changedSize)
			{
				//pipe = create_baked_model_pipeline(window, renderpass.handle, pipeLayout.handle);
				pipe = create_alpha_pipeline(window, renderpass.handle, pipeLayout.handle);
			}
		...
		if (lightFLag)
		{
			...
			{
				sceneUniforms.pointLight[i].lightPosition = update_light_state(sceneUniforms.pointLight[i].lightPosition, dt);
			}
		}
		...
		record_commands(cbuffers[imageIndex], renderpass.handle, framebuffers[imageIndex].handle, pipe.handle, window.swapchainExtent, model, sceneUBO.buffer,
			sceneUniforms, pipeLayout.handle, sceneDescriptors, modelDescriptors, meshbuffer, alphaPipe.handle);
...
namespace
{
	...
	void record_commands(VkCommandBuffer aCmdBuff, VkRenderPass aRenderPass, VkFramebuffer aFramebuffer, VkPipeline aGraphicsPipe, VkExtent2D const& aImageExtent,
		BakedModel& model, VkBuffer aSceneUBO, glsl::SceneUniform const& aSceneUniform, VkPipelineLayout aGraphicsLayout, VkDescriptorSet aSceneDescriptors,
		std::vector<VkDescriptorSet> modelDescriptors, BakedMeshBuffer& meshbuffer, VkPipeline alphaPipe)
	{
		...
		for (size_t i = 0; i < model.meshes.size(); i++)
		{
			...
			if (model.materials[mesh.materialId].alphaMaskTextureId == 0xffffffff)
			{
	...
	glm::vec4 update_light_state(glm::vec3 lightpos, float aElapsedTime)
	{
		...
		theta = rotationSpeed * aElapsedTime;
		...
		glm::vec4 rotatedLightPos = rotationMatrix * glm::vec4(lightpos, 1.0f);

		return glm::vec4(rotatedLightPos);
	}
